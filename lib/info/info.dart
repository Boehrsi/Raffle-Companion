  ...
  Widget build(BuildContext context) {
    return BlocConsumer<InfoCubit, InfoState>(
      ...
      builder: (context, state) {
        if (state is InfoSuccess) {
          return ScaffoldPage(
            header: PageHeader(title: Text(LocaleKeys.infoTitle.tr())),
                      ...
                      children: [
                        RichText(
                          text: TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(
                                text: '${LocaleKeys.infoVersion.tr()} ',
    ...
    context.read<InfoCubit>().openFiles().then((result) {
      ...
      if (!result && safeContext.mounted) {
        showInfoDialog(
          ...
          content: LocaleKeys.errorNoFileExplorer.tr(),
        );
      }
    });
  }
    ...
    context.read<InfoCubit>().sendFeedback().then((result) {
      ...
      if (!result && safeContext.mounted) {
        showInfoDialog(
          context: safeContext,
          title: LocaleKeys.errorTitle.tr(),
          content: LocaleKeys.errorNoMailApp.tr(),
        );
      }
    });
  }
    ...
    context.read<InfoCubit>().openGithub().then((result) {
      ...
      if (!result && safeContext.mounted) {
        showInfoDialog(
          context: safeContext,
          title: LocaleKeys.errorTitle.tr(),
          content: LocaleKeys.errorNoBrowser.tr(),
        );
      }
    });
  }
